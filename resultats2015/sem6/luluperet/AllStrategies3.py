...
##            return SA(V2D(0,yy -self.config.position.y),V2D())
        return SA(yy.position-self.config.position,V2D())
...
#        print(self.autre_player.position.angle,self.autre_player.position.angle-math.pi/2)
    ...
    def check_alert(self):
        ...
        return yh and not self.check_ball() and abs(self.state.ball.vitesse.x) >=0 and not self.state.ball.vitesse.norm <=0.3 and d
