...
#            return SA(V2D(0,yy -self.config.position.y),V2D())
    ...
    def check_attack(self):
        ...
        yp=self.near_of_me()
        jd=self.next_position_ball_state()
        return ((abs(self.state.ball.vitesse.x) <= BALLE_MIN_VITESSE and abs(self.state.ball.vitesse.y) <= BALLE_MIN_VITESSE and yh and self.ball.position.y >= self.goal.y - 2 -GOAL_ATTACK and self.ball.position.y <= self.goal.y + 2 + GOAL_ATTACK and self.state.ball.vitesse.norm <=BALLE_MAX_NORM )or( yh and self.ball.position.y >= self.goal.y - 2 -GOAL_ATTACK and self.ball.position.y <= self.goal.y + 2 + GOAL_ATTACK))  or (self.state.ball.position.distance(self.ma_position) < self.state.ball.position.distance(yp.position) or jd.position.distance(self.ma_position) < jd.position.distance(yp.position))
